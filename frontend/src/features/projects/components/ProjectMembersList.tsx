...
import { GET_PROJECT_MEMBERS, INVITE_TO_PROJECT, REMOVE_MEMBER, UPDATE_MEMBER_ROLE } from '../gql/project-member.graphql';
...
export default function ProjectMembersList({ projectId, currentUserId }: ProjectMembersListProps) {
    ...
    const currentUserMembership = members.find((m: any) => parseInt(m.user.id) === currentUserId);
    ...
    return (
        <div className="bg-white shadow rounded-lg p-6">
            ...
            {isInviteOpen && (
                <form onSubmit={handleInvite} className="mb-4 p-3 bg-gray-50 rounded-md border border-gray-200">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center">
                                {canManage && parseInt(member.user.id) !== currentUserId ? (
                                    <select
                                        value={member.role}
                                        onChange={(e) => handleUpdateRole(parseInt(member.user.id), e.target.value)}
                                    ...
                                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${member.role === 'OWNER' ? 'bg-purple-100 text-purple-700' :
                                ...
                                {canManage && parseInt(member.user.id) !== currentUserId && member.role !== 'OWNER' && (
                                    <button
