...
export class ProjectMemberService {
     ...
     */
    async addMember(projectId: number, userId: number, role: ProjectRole = ProjectRole.MEMBER) {
     ...
     */
    async updateMemberRole(projectId: number, userId: number, newRole: ProjectRole) {
        const member = await this.prisma.projectMember.findUnique({
            where: {
                userId_projectId: { userId, projectId },
            },
     ...
     */
    async checkPermission(userId: number, projectId: number, requiredRoles: ProjectRole[]) {
        const member = await this.prisma.projectMember.findUnique({
            where: {
                userId_projectId: { userId, projectId },
            },
     ...
     */
    async getUserRole(userId: number, projectId: number): Promise<ProjectRole | null> {
        const member = await this.prisma.projectMember.findUnique({
            where: {
                userId_projectId: { userId, projectId },
            },
    ...
    async inviteUser(
        ...
        await this.checkPermission(inviterUserId, projectId, [ProjectRole.OWNER, ProjectRole.ADMIN]);
    ...
    canPerformAction(userRole: ProjectRole, action: string): boolean {
        const permissions = {
            [ProjectRole.OWNER]: ['view', 'create', 'edit', 'delete', 'invite', 'manage_settings', 'manage_members'],
